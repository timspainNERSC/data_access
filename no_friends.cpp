/*
 * The argument class does not need to be friends with ElementData as
 * it provides read-only (const double&) and read-write (double&)
 * access through functions returning references.
 */
...
private:
  ...
  double m_dx_dt;
public:
  Level1IO(ElementData& data) : data(data), m_dx_dt(0.) { };
